...
original_image_path = "white.jpg"  # Replace with the path to your image
...
def count(pixels, col_start, row_start, color):
    ...
    min_shape = min(row_avg.shape, col_avg.shape)
    row_avg = row_avg[:min_shape[0], :min_shape[1], :]
    col_avg = col_avg[:min_shape[0], :min_shape[1], :]

    pixels[col_start + step:col_start - step:step, row_start + step:row_start - 2*step:step, color] = (row_avg + col_avg) // 2
    return pixels
